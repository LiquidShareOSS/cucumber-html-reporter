...
var generateReport = function (options) {
    ...
    var setStats = function (suite) {
        ...
        if (options.attachmentsDirectory) {
            ...
            attachmentsDirectory = options.output ? path.join(options.output, '..', 'attachments') : 'attachments';
        }
        ...
        const featureSetIdCounter = {};
        const featureCounter = {};

        featureOutput.forEach(f => {
            const match = f.name.match(/\[([^\]]+)\] (.*)/);
            if (match && match.length) {
                const setHierarchySource = match[1].split(';');
                const setHierarchy = setHierarchySource.map(s => {
                    const matchNameId = s.match(/([^\()]+)\(\((.*)\)\)/);
                    return {
                        id: matchNameId[1],
                        name: matchNameId[2]
                    }
                });
                const setHierarchyHash = setHierarchy.map(sh => sh.id).join(';');
                const featureName = match[2];

                f.name = featureName;

                const matchId = f.id.match(/\[[^\]]+\]-(.*)/);
                f.id = matchId[1];

                featureCounter[f.id] = featureCounter[f.id] ? featureCounter[f.id] + 1 : 1;
                f.id += featureCounter[f.id];

                if (!setsByHierarchy[setHierarchyHash]) {
                    let currentSetHierarchyHash = '';
                    let currentSetHierarchy;
                    setHierarchy.forEach(set => {
                        if (currentSetHierarchyHash) {
                            currentSetHierarchyHash += ';' + set.id;
                        } else {
                            currentSetHierarchyHash = set.id;
                        }
        ...
        const handleFeatures = (featuresOrSets, level, idHierarchyHash) => {
            ...
            for (let i = 0; i < featuresOrSets.length; i++) {
                ...
                if (featureOrSet.keyword === 'Set') {
                    ...
                    if (!idHierarchyHash) {
                        set.idHierarchyHash = set.id;
                    } else {
                        set.idHierarchyHash = idHierarchyHash + ';' + set.id;
                    }
                    handleFeatures(set.elements, level + 1, set.idHierarchyHash);
                    ...
                    if (!idHierarchyHash) {
                        feature.idHierarchyHash = feature.id;
                    } else {
                        feature.idHierarchyHash = idHierarchyHash + ';' + feature.id;
                    }
                    ...
                    for (let j = 0; j < feature.elements.length; j++) {
                        ...
                        for (let k = 0; k < element.steps.length; k++) {
                            ...
                            if (step.embeddings !== undefined) {
                                step.attachments = [];
                                for (let h = 0; h < step.embeddings.length; h++) {
                                    ...
                                    if (embeddingType === 'application/json') {
                                        var decoded;
                                        ...
                                        if (attachment.name && attachment.content) {
                                            ...
                                            name = name + '_' + attachment.name + '_' + guid() + '.html'; //randomize the file name
                                            var filename = path.join(attachmentsDirectory, name);
                                            fs.writeFileSync(filename, attachment.content, 'utf-8');

                                            step.attachments.push({
                                                name: attachment.name,
                                                path: path.relative(path.join(options.output, '..'), filename)
                                            });
                                        } else {
                                            if (!step.text) {
                                                step.text = decoded;
                                    ...
                                    } else if (['text/plain', 'text/html'].includes(embeddingType)) {
                                        var decoded;
                                        ...
                                        if (options.storeScreenshots && options.storeScreenshots === true) {
                                            ...
                                            name = name + '_' + guid() + '.png'; //randomize the file name
    ...
    let retryReport;
    if (options.jsonDir && options.retryReportFile && fs.existsSync(path.join(options.jsonDir, options.retryReportFile))) {
        retryReport = fs.readFileSync(path.join(options.jsonDir, options.retryReportFile)).toString().replace(/\n/g, '<br/>');
    }
    ...
    fs.writeFileSync(
        ...
        _.template(readFile('index.html'))({
            ...
            screenshot: readFile('../_common/attachments.js'),
